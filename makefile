...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.prismaqf.callblocker"

INSTRUMENTATION_OPTS=" \
--skip-class=com.prismaqf.callblocker.R \
--filter-package=java.io \
--filter-class=java.lang.StringBuilder"

CARVING_OPTIONS=" \
--filter-method com.prismaqf.callblocker.rules.FilterRule.makeRegex \
--filter-method com.prismaqf.callblocker.rules.FilterRule.filterUnwanted \
"
# Default
SED=/usr/bin/sed
UNAME := $(shell uname)
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.prismaqf.callblocker$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.prismaqf.callblocker.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.prismaqf.callblocker > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.prismaqf.callblocker.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.prismaqf.callblocker > /dev/null; \
fi)
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.prismaqf.callblocker.test/androidx.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.prismaqf.callblocker ./traces/$(TEST_NAME) force-clean
